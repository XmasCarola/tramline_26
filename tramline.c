...
void get_station(char * station, int length, char W[7][81], int * index, char selected_direction) {
    Boolean correct_station = FALSE;
    ...
    do {
        ...
        for (w = 0; w < length; w++) {
            ...
            if (entire_string_compare(W[w], station, strLengthInput, strLengthStation) == TRUE) {
                ...
                *index = w;
                if(startOrDestination == 1 && selected_direction == 'I')
...
void create_vector_stations(struct tramline_26 * ptr, char stops[7][81], int size, int timetable_size, const int timetable[7], char selected_direction, const int gaps[], int max_time_gap) {
    int z, gap_sum;
...
int tramline() {
    ...
    char stops[7][81] = {"Amsterdam Centraal", "Muziekgeb. Bimhuis","Kattenburgerstraat", "Rietlandpark", "Zuiderzeeweg", "Bob Haarmslaan", "Steigereiland"};
    ...
    *selected_stop = '\0';
    ...
    get_station(selected_stop, S, stops, &start_index, selected_direction);
    printf("\nYour station is: %s", selected_stop);
    get_direction(&selected_direction, allowed_directions, (sizeof(allowed_directions)/sizeof(allowed_directions[0])), &start_index, S);
    init_tramline_vector(tramline_ptr, S, &selected_direction);
    if(selected_direction=='I')
        create_vector_stations(tramline_ptr, stops, S, (sizeof(inbound_timetable)/sizeof(inbound_timetable[0])), inbound_timetable, selected_direction, time_gaps, max_time_gap);
    else
        create_vector_stations(tramline_ptr, stops, S, (sizeof(outbound_timetable)/sizeof(outbound_timetable[0])), outbound_timetable, selected_direction, time_gaps, max_time_gap);
    printf("\n");
    for(k=0; k<S; k++) {
    ...
    get_station(destination, S, stops, &end_index, selected_direction);
